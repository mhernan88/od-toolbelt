...
def test_multi_filter_by_confidence():
    ...
    coordinates = [np.zeros(coordinates_shape), np.zeros(coordinates_shape), np.zeros(coordinates_shape)]
    confidences = [np.asarray([0.9, 0.4, 0.7]), np.asarray([0.1, 0.3, 0.9]), np.asarray([0.9, 0.9, 0.9])]

    nms = DefaultNonMaximumSuppression(0, 0.5)
    out_coordinates, out_confidences, out_coordinates_combined, out_confidences_combined = \
